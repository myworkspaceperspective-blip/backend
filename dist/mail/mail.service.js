...
let MailService = MailService_1 = class MailService {
    ...
    async sendVerificationEmail(to, name, token) {
        const url = `${this.config.getOrThrow('FRONTEND_URL')}/verify-email/${token}`;
        try {
            await this.transporter.sendMail({
                from: `"${this.config.get('MAIL_FROM_NAME', 'App')} " <${this.config.getOrThrow('SMTP_USER')}>`,
                to,
    ...
    async sendPasswordResetEmail(to, name, token) {
        const url = `${this.config.getOrThrow('FRONTEND_URL')}/reset-password/${token}`;
        try {
            await this.transporter.sendMail({
                from: `"${this.config.get('MAIL_FROM_NAME', 'App')} " <${this.config.getOrThrow('SMTP_USER')}>`,
                to,
